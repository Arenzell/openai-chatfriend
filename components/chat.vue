...
function typeResponse(response) {
  ...
  // Define a function to add each character to the output element with a delay
  let i = 0;
  function addChar() {
    if (i < responseChars.length && response.type === 1) {
      outputElement.textContent += responseChars[i];
      // chatMessages.value.push(responseChars[i]);
  ...
  // Start the typing animation by scheduling the first character
  setTimeout(addChar, 1000); // Wait for 1 second before starting the typing animation
}
